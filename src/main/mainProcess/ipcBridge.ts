...
import { isExistingDirectory, resolveWithinDir, safeModuleName, safeJsonFilename } from './pathSafety'
import {
...
import { MODULE_METADATA_MAX_BYTES } from '../../constants'

import type { PathLike } from 'fs'
import type { IpcMainEvent, IpcMainInvokeEvent } from 'electron'
import type { Jsonish } from '../../types/utils'

const getProjectDirForEvent = (event: IpcMainEvent | IpcMainInvokeEvent) => {
  try {
...
const readFileHeadUtf8 = async (filePath: string, maxBytes: number) => {
  let fh
  try {
...
export function registerIpcBridge() {
  ...
  ipcMain.handle('bridge:workspace:readModuleWithMeta', async (event, moduleName: string) => {
    const projectDir = getProjectDirForEvent(event)
  ...
  ipcMain.handle('bridge:workspace:getModuleUrl', async (event, moduleName: string) => {
    const projectDir = getProjectDirForEvent(event)
    if (!projectDir || !isExistingDirectory(projectDir)) return null
    const safe = safeModuleName(moduleName)
    if (!safe) return null
  ...
  ipcMain.handle('bridge:workspace:readModuleText', async (event, moduleName: string) => {
    const projectDir = getProjectDirForEvent(event)
    if (!projectDir || !isExistingDirectory(projectDir)) return null
    const safe = safeModuleName(moduleName)
    if (!safe) return null
  ...
  ipcMain.on('bridge:workspace:writeModuleTextSync', (event, moduleName: string, text: string) => {
    const projectDir = getProjectDirForEvent(event)
  ...
  ipcMain.on('bridge:workspace:moduleExists', (event, moduleName: string) => {
    const projectDir = getProjectDirForEvent(event)
    if (!projectDir || !isExistingDirectory(projectDir)) {
  ...
  ipcMain.on('bridge:workspace:showModuleInFolder', (event, moduleName: string) => {
    const projectDir = getProjectDirForEvent(event)
    if (!projectDir || !isExistingDirectory(projectDir)) return
    const safe = safeModuleName(moduleName)
    if (!safe) return
  ...
  ipcMain.on('bridge:workspace:assetUrl', (event, relPath: PathLike) => {
    const projectDir = getProjectDirForEvent(event)
    if (!projectDir || !isExistingDirectory(projectDir)) {
  ...
  ipcMain.handle('bridge:workspace:listAssets', async (event, relDir: PathLike) => {
    const projectDir = getProjectDirForEvent(event)
    if (!projectDir || !isExistingDirectory(projectDir)) {
  ...
  ipcMain.handle('bridge:workspace:readAssetText', async (event, relPath: PathLike) => {
    const projectDir = getProjectDirForEvent(event)
    if (!projectDir || !isExistingDirectory(projectDir)) return null
    const assetsDir = path.join(projectDir, 'assets')
    const fullPath = resolveWithinDir(assetsDir, String(relPath || ''))
  ...
  ipcMain.handle('bridge:json:read', async (event, filename: string, defaultValue: Jsonish) => {
    const projectDir = getProjectDirForEvent(event)
  ...
  ipcMain.on('bridge:json:readSync', (event, filename: string, defaultValue: Jsonish) => {
    const projectDir = getProjectDirForEvent(event)
    const safeName = safeJsonFilename(filename)
    if (!safeName) {
  ...
  ipcMain.handle('bridge:json:write', async (event, filename: string, data: any) => {
    const projectDir = getProjectDirForEvent(event)
    const safeName = safeJsonFilename(filename)
  ...
  ipcMain.on('bridge:json:writeSync', (event, filename: string, data: any) => {
    const projectDir = getProjectDirForEvent(event)
    const safeName = safeJsonFilename(filename)
    if (!safeName) {
  ...
  // TODO - check for a better way instead of this call
  ipcMain.on('bridge:app:getBaseMethodNames', (event) => {
    try {
      const moduleBasePath = path.join('projector', 'helpers', 'moduleBase.js')
      const threeBasePath = path.join('projector', 'helpers', 'threeBase.js')
  ...
  ipcMain.on('bridge:app:getRepositoryUrl', (event) => {
    try {
      event.returnValue = repository.url
    } catch {
      event.returnValue = null
    }
  })
  ...
  ipcMain.on('bridge:app:getKickMp3ArrayBuffer', (event) => {
    try {
      const kickPath = 'dashboard/assets/audio/kick.mp3'
  ...
  ipcMain.on('bridge:os:clipboardWriteText', (event, text: string) => {
    try {
  ...
  ipcMain.on('bridge:os:openExternal', (event, url: string) => {
    try {
  ...
  ipcMain.handle('input:configure', async (_event, payload) => {
  ...
  ipcMain.on('log-to-main', (_event, message: any) => {
    console.log(message)
  })
}
