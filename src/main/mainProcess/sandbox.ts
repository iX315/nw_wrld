...
import { SANDBOX_ASSET_TEXT_MAX_BYTES } from '../../constants'
...
const readFileUtf8WithLimit = async (filePath: PathLike, maxBytes: number) => {
  try {
...
const getProjectDirForEvent = (event: IpcMainInvokeEvent) => {
  try {
...
export const registerSandboxToken = (event: IpcMainEvent | IpcMainInvokeEvent, token: string, projectDir: string) => {
...
export const unregisterSandboxToken = (token: string) => {
  const safeToken = String(token || '').trim()
...
export const updateSandboxViewBounds = () => {
  if (
    !state.sandboxView ||
...
export const destroySandboxView = () => {
...
export const ensureSandboxView = (projectDir: string) => {
  ...
  state.sandboxView = new BrowserView({
    webPreferences: {
      ...
      sandbox: true,
      preload: path.join('sandboxPreload.js'),
...
const isProjectorEvent = (event: IpcMainEvent | IpcMainInvokeEvent) => {
  try {
    const senderId = event?.sender?.id
    return (
      typeof senderId === 'number' &&
...
const destroySandboxForProjector = (ownerWebContentsId: number | null) => {
  if (state.activeSandboxToken) {
    try {
      unregisterSandboxToken(state.activeSandboxToken)
    } catch {}
...
export function registerSandboxIpc() {
  ...
  ipcMain.handle('sandbox:destroy', async (event) => {
    if (!isProjectorEvent(event)) return { ok: false, reason: 'FORBIDDEN' }
    const ownerId = typeof event?.sender?.id === 'number' ? event.sender.id : null
