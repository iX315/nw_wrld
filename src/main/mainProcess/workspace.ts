...
const getLegacyJsonDirForMain = () => path.join('', '..', 'src', 'shared', 'json')

const getFallbackJsonDirForMain = () => path.join('', 'shared', 'json')
...
export const maybeMigrateLegacyJsonFileForBridge = (projectDir: string, filename: string) => {
...
export const maybeMigrateJsonIntoProject = (projectDir: string) => {
...
export const startWorkspaceWatcher = (workspacePath) => {
...
export const ensureWorkspaceScaffold = async (workspacePath) => {
...
export function registerWorkspaceSelectionIpc({ createWindow }) {
